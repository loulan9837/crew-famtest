...
def generate_incremental_cases(
    ...
    prompt.append(
        "5. 前置条件不得写用户已登录、群主已登录、App已打开、应用已启动等默认基础前提，只写本条用例特有数据、权限或界面状态。"
    )
    prompt.append("6. 必须显式对比现有测试用例，严禁重复已有场景。")
    prompt.append("7. 必须考虑断网、杀进程、边界值等极端场景，以及新逻辑对既有流程的潜在回归风险。")
    prompt.append("8. 若发现增量指令引入的新行为与原用例中已有行为存在冲突，必须补充回归/冲突验证用例。")
    prompt.append("")
